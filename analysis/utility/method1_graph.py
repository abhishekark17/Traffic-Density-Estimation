...
for i in range(len(df_time["method"])):
    ...
    if(len(x)>5):
        if x[5]=="1":
            time.append(int(t))
            frameSkip.append(x[7:])


def main(frameSkip):
    filename="../data/data1/dataM1F"+str(frameSkip)+".csv"
...
def graph_preprocess():
    ...
    for i in range(len(time)):
        error_q,error_d=main(frameSkip[i])
...
def trade_off_graph(type_of_graph):
    ...
    for i in range(len(time)):
        marker_q.append("QF"+str(frameSkip[i]))
        marker_d.append("DF"+str(frameSkip[i]))


    fig,ax=plt.subplots()
    if type_of_graph=="queue": # only queue error
        ax.scatter(error_queue,time)
        fig.suptitle("Queue Density Error (Method 1)")
        ax.set_xlabel("Error")
        ax.set_ylabel("Time")
        ...
        fig.suptitle("Dynamic Density Error (Method 1)")
        ax.set_xlabel("Error")
        ax.set_ylabel("Time")
        ...
        ax.scatter(error_queue,time)
        fig.suptitle("Both Queue and Dynamic Density Error (Method 1)")
        ax.set_xlabel("Error")
        ax.set_ylabel("Time")
        #plt.xlim(0,max(error_dynamic)*1.2)
        #ax.annotate("Normal Time = "+str(Normal_time),(min(error_dynamic)*1.2,Normal_time))
    ...
    plt.savefig("../graphs/"+"method1_tradeoff_"+type_of_graph+".png")
    plt.show()
